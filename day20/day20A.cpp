...
void busca_camino_con_atajo(std::vector<std::string> &mapa, int f, int c, std::vector<std::vector<int>> &camino, int &num_lineas, int &num_columnas, int profundidad, unsigned long long &total) {
    ...
    if (c < num_columnas - 3) {
        if (camino [f][c + 1] < 0 && camino [f][c + 2] > profundidad) {
            int mejora = camino [f][c + 2] - profundidad - 2;
            #ifdef LOG_CHECK
    ...
    if (f < num_lineas - 3) {
        if (camino [f + 1][c] < 0 && camino [f + 2][c] > profundidad) {
            int mejora = camino [f + 2][c] - profundidad - 2;
            #ifdef LOG_CHECK
                printf("Mejora abajo %i,%i: %i\n", f, c, mejora);
            #endif
            if (mejora >= MIN_MEJORA) {
                total++;
            }
